...
@implementation MCOSAMKeychainQuery

@synthesize mcoAccount = _mcoAccount;
@synthesize mcoService = _mcoService;
@synthesize mcoLabel = _mcoLabel;
@synthesize mcoPasswordData = _mcoPasswordData;

#ifdef SAMKEYCHAIN_ACCESS_GROUP_AVAILABLE
@synthesize mcoAccessGroup = _mcoAccessGroup;
#endif

#ifdef SAMKEYCHAIN_SYNCHRONIZATION_AVAILABLE
@synthesize mcoSynchronizationMode = _mcoSynchronizationMode;
#endif
...
- (BOOL)save:(NSError *__autoreleasing *)error {
    ...
    if (!self.mcoService || !self.mcoAccount || !self.mcoPasswordData) {
        if (error) {
    ...
    if (status == errSecSuccess) {//item already exists, update it!
        ...
        [query setObject:self.mcoPasswordData forKey:(__bridge id)kSecValueData];
#if __IPHONE_4_0 && TARGET_OS_IPHONE
        CFTypeRef accessibilityType = [MCOSAMKeychain accessibilityType];
        ...
        query = [self query];
        if (self.mcoLabel) {
            [query setObject:self.mcoLabel forKey:(__bridge id)kSecAttrLabel];
        }
        [query setObject:self.mcoPasswordData forKey:(__bridge id)kSecValueData];
#if __IPHONE_4_0 && TARGET_OS_IPHONE
        CFTypeRef accessibilityType = [MCOSAMKeychain accessibilityType];
...
- (BOOL)deleteItem:(NSError *__autoreleasing *)error {
    OSStatus status = SAMKeychainErrorBadArguments;
    if (!self.mcoService || !self.mcoAccount) {
        if (error) {
            *error = [[self class] errorWithCode:status];
        }
        return NO;
...
- (BOOL)fetch:(NSError *__autoreleasing *)error {
    ...
    self.mcoPasswordData = (__bridge_transfer NSData *)result;
    return YES;
}
...
- (void)setPasswordObject:(id<NSCoding>)object {
    self.mcoPasswordData = [NSKeyedArchiver archivedDataWithRootObject:object];
}


- (id<NSCoding>)passwordObject {
    if ([self.mcoPasswordData length]) {
        return [NSKeyedUnarchiver unarchiveObjectWithData:self.mcoPasswordData];
    }
    return nil;
}
...
- (void)setPassword:(NSString *)password {
    self.mcoPasswordData = [password dataUsingEncoding:NSUTF8StringEncoding];
}


- (NSString *)password {
    if ([self.mcoPasswordData length]) {
        return [[NSString alloc] initWithData:self.mcoPasswordData encoding:NSUTF8StringEncoding];
    }
    return nil;
}
...
- (NSMutableDictionary *)query {
    ...
    if (self.mcoService) {
        [dictionary setObject:self.mcoService forKey:(__bridge id)kSecAttrService];
    }

    if (self.mcoAccount) {
        [dictionary setObject:self.mcoAccount forKey:(__bridge id)kSecAttrAccount];
    }

#ifdef SAMKEYCHAIN_ACCESS_GROUP_AVAILABLE
#if !TARGET_IPHONE_SIMULATOR
    if (self.mcoAccessGroup) {
        [dictionary setObject:self.mcoAccessGroup forKey:(__bridge id)kSecAttrAccessGroup];
    }
#endif
#endif
    ...
    if ([[self class] isSynchronizationAvailable]) {
        ...
        switch (self.mcoSynchronizationMode) {
...
+ (NSError *)errorWithCode:(OSStatus) code {
    ...
    dispatch_once(&onceToken, ^{
        NSURL *url = [[NSBundle bundleForClass:[MCOSAMKeychainQuery class]] URLForResource:@"SAMKeychain" withExtension:@"bundle"];
    ...
    return [NSError errorWithDomain:mcoKSAMKeychainErrorDomain code:code userInfo:userInfo];
}
